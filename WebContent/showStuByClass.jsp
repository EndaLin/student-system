						...
						<tbody>
							<c:forEach var="p" items="${sublist}" varStatus="status"  begin="<%=page_begin %>" end="<%=page_end %>">
								<tr>
									...
									<td>${p.cname}</td>
									<td><a
										href="/StuSys/ModifyStudent.jsp?id=${p.sid }&name=${p.sname}&cid=${p.cid}&cname=${p.cname}&cgrade=${p.cgrade}"><i
								...
								<div class="modal small hide fade" id="myModal" tabindex="-1"
									...
									<div class="modal-footer">
										...
										<button class="btn" data-dismiss="modal"
											onClick="window.open('/StuSys/DeleteStudentByIdServlet?id=${p.sid}')">Delete</button>
									</div>
								</div>
				...
				<div class="pagination">
					<ul>
					  <c:if test="${sessionScope.page_current != 1 }">
						<li><a href="/StuSys/showStuByClass.jsp?begin=${sessionScope.page_current - 1 }">Prev</a></li>
						</c:if>
						<c:if test="${sessionScope.page_current != sessionScope.page_total }">
						<li><a href="/StuSys/showStuByClass.jsp?begin=${sessionScope.page_current + 1 }">Next</a></li>
						</c:if>
					</ul>
