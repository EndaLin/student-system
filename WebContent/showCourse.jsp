...
<body class="">
			...
			<ul class="nav pull-right">
				...
				<li id="fat-menu" class="dropdown"><a href="#" role="button"
					class="dropdown-toggle" data-toggle="dropdown"> <i
						class="icon-user"></i> ${user.account} <i class="icon-caret-down"></i>
				</a>
	...
	<div class="sidebar-nav">
		...
		<c:if test="${user.type == 0 }">
			<a href="#dashboard-menu" class="nav-header" data-toggle="collapse"><i
				class="icon-dashboard"></i>信息管理<i class="icon-chevron-up"></i></a>
		...
		<c:if test="${user.type == 1 }">
			<a href="#accounts-menu" class="nav-header" data-toggle="collapse"><i
				class="icon-briefcase"></i>学生成绩管理<i class="icon-chevron-up"></i></a>
	...
	<div class="content">
		...
		<form action="/StuSys/AddCourse" method="post"
		...
		<hr />
		<form action="/StuSys/SaveExcelServlet" id="tab" method="post"
						...
						<tbody>
							<c:forEach var="p" items="${slist}" varStatus="status"
								...
								<tr>
									...
									<td>${p.name}</td>
									<td><a
										href="/StuSys/ModifyCourseName.jsp?id=${p.id }&name=${p.name}"><i
								...
								<div class="modal small hide fade" id="myModal" tabindex="-1"
									...
									<div class="modal-footer">
										...
										<button class="btn" data-dismiss="modal"
											onClick="window.open('/StuSys/DeleteCourseServlet?id=${p.id}')">Delete</button>
									</div>
								</div>
				...
				<div class="pagination">
					<ul>
						<c:if test="${sessionScope.page_current != 1 }">
							<li><a
								href="/StuSys/showCourse.jsp?begin=${sessionScope.page_current - 1 }">Prev</a></li>
						</c:if>
						<c:if
							test="${sessionScope.page_current != sessionScope.page_total }">
							<li><a
								href="/StuSys/showCourse.jsp?begin=${sessionScope.page_current + 1 }">Next</a></li>
						</c:if>
					</ul>
