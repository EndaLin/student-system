...
<%@ page language="java" import="me.domain.Course"%>
...
<html>
	...
	<body class="">
		<!-- 分页操作-->
		<%
		ArrayList<Course> list = (ArrayList<Course>) request.getSession().getAttribute("slist");
	...
	%>
			...
			<div class="navbar-inner">
				<ul class="nav pull-right">
					...
					<li id="fat-menu" class="dropdown">
						<a href="#" role="button" class="dropdown-toggle" data-toggle="dropdown"> <i class="icon-user"></i> ${user.account} <i class="icon-caret-down"></i>
						</a>
				...
				<a class="brand" href="/StuSys/index.jsp"><span class="first">学生信息</span>
		...
		<div class="content">
			...
			<form action="/StuSys/AddCourse" method="post" style="margin-left: 20px;">
			...
			<hr />
			<form action="/StuSys/SaveExcelServlet" id="tab" method="post" enctype="multipart/form-data">
				<input type="hidden" name="path" value="/showCourse.jsp" /> <label>批量上传课程信息(请上传Excel表，课程名称</label>
				...
				<div class="row-fluid">
					...
					<div class="well">
						<table class="table">
							...
							<tbody>
								<c:forEach var="p" items="${slist}" varStatus="status" begin="<%=page_begin %>" end="<%=page_end %>">
									<tr>
										<td>${p.id}</td>
										<td>${p.name}</td>
										<td>
											<a href="/StuSys/ModifyCourseName.jsp?id=${p.id }&name=${p.name}"><i class="icon-pencil"></i></a>
									...
									<div class="modal small hide fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
										...
										<div class="modal-footer">
											...
											<button class="btn" data-dismiss="modal" onClick="window.open('/StuSys/DeleteCourseServlet?id=${p.id}')">Delete</button>
										</div>
									</div>
					...
					<div class="pagination">
						<ul>
							<c:if test="${sessionScope.page_current != 1 }">
								<li>
									<a href="/StuSys/showCourse.jsp?begin=${sessionScope.page_current - 1 }">Prev</a>
								</li>
							</c:if>
							<c:if test="${sessionScope.page_current != sessionScope.page_total }">
								<li>
									<a href="/StuSys/showCourse.jsp?begin=${sessionScope.page_current + 1 }">Next</a>
								</li>
							</c:if>
						</ul>
